...
class AuthService {
    ...
    async signUp(signUpDTO: SignUpDTO){
        ...
        const existingUser = await databaseClient.db.select().from(usersTable)
                            .where(eq(usersTable.email, email));
        if (existingUser.length > 0) {
            throw new Error("User with this email already exists.");
        }
        const hashedPassword = await bcrypt.hash(password, 10);
        const newUser = await databaseClient.db.insert(usersTable)
                        .values({ firstName: firstName, lastName: lastName, email: email, password: hashedPassword })
    ...
    async logIn(logInDTO: LogInDTO) {
        ...
        const user = await databaseClient.db
                    .select({userId: usersTable.userId, email: usersTable.email, password: usersTable.password, role: usersTable.role, isActive: usersTable.isActive })
        ...
        await databaseClient.db.insert(refreshTokensTable)  
        .values({ userId: user[0].userId, tokenHash: hashedToken, expiresAt: new Date(Date.now() + 7 * 24 * 60 * 60 * 1000) });
    ...
    async logOut(refreshTokenDTO: RefreshTokenDTO) {
        ...
        const userTokens = await databaseClient.db.select().from(refreshTokensTable)
        .where(and
                (eq(refreshTokensTable.userId, decoded.userId),
                eq(refreshTokensTable.isRevoked, false),
                gt(refreshTokensTable.expiresAt, new Date())))
        .execute();
        const validToken = userTokens.find(token => bcrypt.compare(refreshToken, token.tokenHash));
        if (!validToken) {
            throw new Error("Invalid refresh token.");
        }
        await databaseClient.db.update(refreshTokensTable).set({ isRevoked: true }).where(eq(refreshTokensTable.id, validToken.id));   
    }
